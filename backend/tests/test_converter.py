...
def test_create_lithophane_outputs_watertight_files(tmp_path: Path, name: str, size: tuple[int, int], mode: str) -> None:
    result = create_lithophane(_image_bytes(size, mode), "Medium", tmp_path, name)
...
@pytest.mark.parametrize(("size", "expected_mm"), [("Small", 100.0), ("Medium", 150.0), ("Large", 200.0)])
...
@pytest.mark.parametrize(
    ...
    [
        ("Portrait", 130.0, 173.0),
        ("Landscape", 173.0, 130.0),
        ("Square", 150.0, 150.0),
    ],
)
def test_orientation_controls_dimensions(
    ...
    result = create_lithophane(_image_bytes((120, 80), "RGB"), "Medium", tmp_path, orientation, orientation)
    mesh = trimesh.load_mesh(result.stl_path)
...
def test_invalid_image_raises(tmp_path: Path) -> None:
    with pytest.raises(InvalidImageError):
        create_lithophane(b"not an image", "Small", tmp_path, "bad")
