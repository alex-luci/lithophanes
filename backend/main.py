...
from .converter import InvalidImageError, LampSize, LithophaneOrientation, create_lithophane
...
async def create_lithophane_job(
    ...
    size: LampSize = Form(...),
    orientation: LithophaneOrientation = Form("Square"),
) -> dict[str, str]:
    ...
    background_tasks.add_task(_submit_conversion, job_id, image_bytes, size, orientation)
...
def _submit_conversion(job_id: str, image_bytes: bytes, size: LampSize, orientation: LithophaneOrientation) -> None:
    jobs[job_id] = {"jobId": job_id, "status": "processing"}
    future = executor.submit(create_lithophane, image_bytes, size, GENERATED_DIR, job_id, orientation)
