...
def create_lithophane(image_bytes: bytes, size: LampSize, output_dir: Path, stem: str) -> LithophaneResult:
    if size not in SIZE_CONFIG:
        raise ValueError(f"Unknown size: {size}")

    output_dir.mkdir(parents=True, exist_ok=True)
    image = _load_grayscale(image_bytes)
    heightmap, width_mm, height_mm = _image_to_heightmap(image, SIZE_CONFIG[size])
...
def _image_to_heightmap(image: Image.Image, max_dimension_mm: float) -> tuple[np.ndarray, float, float]:
